...
def find_arch_sqlite (base, serv):
  ...
  try:
    ...
    for row in cursor.fetchall():
      if ('_'+year_n+'_'+'%02d'% (int(m_n))) in row[0]:
        print('Сервер: '+serv+'; база: '+os.path.split(base)[1]+';  архив:  ' +row[0] + ' suschestvuet ', file=file_op)
        flag=1
    ...
    for line in open( os.curdir +'\\list_server_table.txt' , 'r' ) : #забираем из файла конфигурации данные для функции перезапуска
      ...
      if len(line) > 10:
        ...
        if len(line.split(' ')) > 3:
          ...
          find_arch_sqlite(line.split(' ')[0],line.split(' ')[1].rstrip('\n'))
