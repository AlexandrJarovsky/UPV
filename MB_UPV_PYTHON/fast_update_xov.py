    ...
    try:
        ...
        for st in cursor.fetchall():
            ...
            for sh in str(conf_dict['shifrs']).split(','):
                Zn = 'Zn_' + sh.split('_')[1]
                if st[d_c['Prizn']] not in ['NEFT_OUT_8']:
            ...
            if result >= 0 and st[d_c['Prizn']] != 'sep':
                ...
                elif st[d_c['Prizn']] in ['NEFT_IN7', 'NEFT_IN']:
                    pass

                elif st[d_c['Prizn']] in ['NEFT_IN4', 'NEFT_IN2']:
                    Sz_hour_m=Density=0
                    list_zn0 = list_zn[0] if list_zn[0] >=0 else 0
                    list_zn1 = list_zn[1] if list_zn[1] >=0 else 0

                    t1=list_zn0/1000
                    t2=list_zn1 * (st[d_c['P_nom']] / 1000 - (0.001828 - 0.00132 * st[d_c['P_nom']] / 1000) * (list_zn[3] - 20))
                    t3=(list_zn0/1000 - list_zn1 * (st[d_c['P_nom']] / 1000 - 
                        (0.001828 - 0.00132 * st[d_c['P_nom']] / 1000) * (list_zn[3] - 20))- st[d_c['Sz_hour_m']]) / (st[d_c['n']] + 1)

                    Sz_hour_m = st[d_c['Sz_hour_m']] + (list_zn0/1000 -
                        list_zn1 * (st[d_c['P_nom']] / 1000 - (0.001828 - 0.00132 * st[d_c['P_nom']] / 1000) * (list_zn[3] - 20))- st[d_c['Sz_hour_m']]) / (st[d_c['n']] + 1)

                    Density = (st[d_c['P_nom']] /1000 - (0.001828 - 0.00132 * st[d_c['P_nom']]/1000) * (
                        list_zn[3] - 20)) * 1000

                    cursor.execute("""UPDATE %s SET Tzn = %s WHERE Shifr='%s'""" % (conf_dict['Table_work'], Sz_hour_m, st[d_c['Shifr']]))
                    cursor.execute("""UPDATE %s SET Sz_hour_m = %s WHERE Shifr='%s'""" % (
                        conf_dict['Table_work'], Sz_hour_m, st[d_c['Shifr']]))
                    cursor.execute("""UPDATE %s SET Density = %s WHERE Shifr='%s'""" % (
        ...
        for st in cursor.fetchall():
            ...
            if st[d_c['Prizn']] in ['NEFT_IN1', 'NEFT_IN2', 'NEFT_IN4',  'NEFT_IN9', 'NEFT_IN5', 'NEFT_IN10', 'NEFT_IN3', 'NEFT_IN8', 'NEFT_IN6', 'NEFT_IN11']:
                m_in += st[d_c['Sz_hour_m']]

        temp_rasch = m_in*(potoc[d_potoc['vodorod']]/100)

        cursor.execute("""UPDATE %s SET Sz_hour_m = %s WHERE Shifr='%s'""" % (conf_dict['Table_work'], temp_rasch, 'FIC3003'))
        ...
        for st in cursor.fetchall():
            ...
            if st[d_c['Prizn']] in ['NEFT_OUT']:
                m_out += st[d_c['Sz_hour_m']]

            if st[d_c['Prizn']] in ['NEFT_IN1', 'NEFT_IN2', 'NEFT_IN4',  'NEFT_IN9', 'NEFT_IN5', 'NEFT_IN10', 'NEFT_IN3', 'NEFT_IN8', 'NEFT_IN6', 'NEFT_IN11']:
                m_in += st[d_c['Sz_hour_m']]

        temp_rasch = m_in - m_out - 0.006*m_in

        cursor.execute("""UPDATE %s SET Sz_hour_m = %s WHERE Shifr='%s'""" % (conf_dict['Table_work'], temp_rasch, 'FIC3002'))
