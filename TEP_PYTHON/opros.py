...
def opros(proc_rasch=1, lab_density=1):
    try:
...
#        if not Flag_prn:
            ...
            if len(cursor.execute("SELECT * FROM seting").fetchall()) != 0:
                tep_print.prin_pr(flag_prn=int(set_print[0]))
#                threading.Thread(target=lambda: tep_print.prin_pr(flag_prn=set_print[0]),args=()).start()
            else:
                tep_print.prin_pr()
#                threading.Thread(target=lambda: tep_print.prin_pr(),args=()).start()
...
#        if not HFlag :
            ...
            try:
                cursor.execute('SELECT * FROM table1 order by sortorder')
                for st in cursor.fetchall():
                    for i in range(1, 32):
                        cursor.execute("UPDATE %s SET Sz_N" % (conf_dict['Table_work']) + str(
